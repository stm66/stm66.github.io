### [Главная страница](https://stm66.github.io/)

## Руководства по сборке своего дистрибутива
- [Linux for ARM - руководство по сборке своего дистрибутива Linux из исходного кода для ARM-компьютеров](https://github.com/Linux-for-ARM)
- [Linux для себя](https://lx4u.ru/)
- [Cross-Compiled Linux From Scratch - Embedded ](http://clfs.org/view/clfs-embedded/arm/)
- [Linux for ARM (сама книга)](https://linux-for-arm.github.io/)

## Gentoo
- [gentoo arm64](https://wiki.gentoo.org/wiki/Project:ARM64)
- [Embedded Handbook/General/Compiling with QEMU user chroot](https://wiki.gentoo.org/wiki/Embedded_Handbook/General/Compiling_with_QEMU_user_chroot#Usage)

## Radxa
- [Radxa](work/radxa.md)

## Luckfox

## Полезные ссылки
- [BINFMT_MISC](https://en.wikipedia.org/wiki/Binfmt_misc)
- [Kernel Support for miscellaneous Binary Formats (binfmt_misc)](https://www.kernel.org/doc/Documentation/admin-guide/binfmt-misc.rst)
- [Compiling with qemu user chroot](https://wiki.gentoo.org/wiki/Embedded_Handbook/General/Compiling_with_qemu_user_chroot)
- [Building Ubuntu rootfs for ARM](https://gnu-linux.org/building-ubuntu-rootfs-for-arm.html)
- [How to create a custom Ubuntu live from scratch](https://itnext.io/how-to-create-a-custom-ubuntu-live-from-scratch-dd3b3f213f81)
- [Crossdev qemu-static-user-chroot](https://wiki.gentoo.org/wiki/Crossdev_qemu-static-user-chroot)
- [Etckeeper](https://wiki.archlinux.org/index.php/Etckeeper)

https://en.wikipedia.org/wiki/Binfmt_misc

### [Главная страница](https://stm66.github.io/)